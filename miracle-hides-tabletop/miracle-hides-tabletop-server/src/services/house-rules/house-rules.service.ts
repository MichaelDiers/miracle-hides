...
import { HOUSE_RULES_DATABASE_SERVICE, IHouseRulesDatabaseService } from 'src/types/house-rules-database-service.interface';
...
export class HouseRulesService implements IHouseRulesService {
  constructor(
    @Inject(HOUSE_RULES_DATABASE_SERVICE) private readonly houseRulesDatabaseService: IHouseRulesDatabaseService,
  ) {
  }

  readAsync(language: Language): Promise<IReadHouseRulesResult|null> {
    return new Promise((resolve, reject) => {
      this.houseRulesDatabaseService.read(language)
        .then((result) => {
          if (!result) {
            reject('not found');
          }
