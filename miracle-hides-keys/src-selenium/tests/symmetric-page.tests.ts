...
import { SymmetricPage, SymmetricPageValues } from '../pages/symmetric-page';
  ...
  TestFrame.testFrames().forEach((testFrameEntry) => {
    describe(TestFrame.displayName(testFrameEntry), () => {
      ...
      it('check initial values', async function test() {
        ...
        const page : SymmetricPage = this.page;
        const values : SymmetricPageValues = await page.collectValuesAsync();
