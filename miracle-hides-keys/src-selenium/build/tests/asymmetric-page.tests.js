...
const asymmetricPageValuesCompare = (oldValues, newValues, oldRsaKeySize, newRsaKeySize, oldAlgorithm, newAlgorithm) => {
    ...
    [{
        ...
        if (algorithm === 'EC') {
            assert.match(data.privateKey, /^-----BEGIN EC PRIVATE KEY-----.+-----END EC PRIVATE KEY-----\s*$/gms);
            assert.match(data.publicKey, /^-----BEGIN PUBLIC KEY-----.+-----END PUBLIC KEY-----\s*$/gms);
            assert.equal(data.inputTextDecrypted, 'true');
        }
        else {
            assert.match(data.privateKey, /^-----BEGIN RSA PRIVATE KEY-----.+-----END RSA PRIVATE KEY-----\s*$/gms);
            assert.match(data.publicKey, /^-----BEGIN RSA PUBLIC KEY-----.+-----END RSA PUBLIC KEY-----\s*$/gms);
    ...
    test_frame_1.TestFrame.testFrames().forEach((testFrameEntry) => {
        describe(test_frame_1.TestFrame.displayName(testFrameEntry), () => {
            ...
            it('check initial values', async function test() {
                ...
                assert.match(values.privateKey, /^-----BEGIN EC PRIVATE KEY-----.+-----END EC PRIVATE KEY-----\s*$/gms);
                assert.match(values.publicKey, /^-----BEGIN PUBLIC KEY-----.+-----END PUBLIC KEY-----\s*$/gms);
